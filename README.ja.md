...
内部システムは `X-Key-Id` / `X-Key` ヘッダーで `/api/internal/*` を呼び出します。キーペアは **システム設定 → 認証設定 → 内部システム認証** で構成します。キーに IP ホワイトリストを設定した場合、**TCP 直接接続のピア IP** で判定されます（`X-Forwarded-For` などの転送ヘッダーは無視されます）。リバースプロキシ経由ではすべてのリクエストの送信元がプロキシの IP になるため、プロキシの IP をホワイトリストに追加するか、プロキシ層で `/api/internal/` へのアクセスを制限してください。この判定は転送ヘッダーのクライアント IP から TCP 直接接続のピア IP に変更されました。アップグレード時は、プロキシ背後の実際のクライアント IP 向けに設定していたホワイトリストのエントリをプロキシの IP に置き換えてください。
