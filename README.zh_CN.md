...
内部系统通过 `X-Key-Id` / `X-Key` 请求头调用 `/api/internal/*`，密钥对在 **系统设置 → 鉴权设置 → 内部系统鉴权** 中配置。若密钥设置了 IP 白名单，将按 **TCP 直连对端 IP** 校验（忽略 `X-Forwarded-For` 等转发头）。反向代理部署下所有请求的来源都是代理 IP——请将代理 IP 加入白名单，或在代理层限制对 `/api/internal/` 的访问。此校验已从转发头中的客户端 IP 改为 TCP 直连对端 IP：升级后请将此前按真实客户端 IP 配置的白名单条目改为代理 IP。
