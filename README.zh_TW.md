...
內部系統透過 `X-Key-Id` / `X-Key` 請求標頭呼叫 `/api/internal/*`，密鑰對在 **系統設定 → 鑑權設定 → 內部系統鑑權** 中配置。若密鑰設置了 IP 白名單，將按 **TCP 直連對端 IP** 驗證（忽略 `X-Forwarded-For` 等轉發標頭）。反向代理部署下所有請求的來源都是代理 IP——請將代理 IP 加入白名單，或在代理層限制對 `/api/internal/` 的存取。此驗證已從轉發標頭中的用戶端 IP 改為 TCP 直連對端 IP：升級後請將此前按真實用戶端 IP 配置的白名單條目改為代理 IP。
